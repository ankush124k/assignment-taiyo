...
const EditContact = ({ id }: Props) => {
  ...
  const filteredData = data.filter((el: ContactType) => el.id === id);
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!contact.first || !contact.last || !contact.status) {
        ...
        const newData = data.map((el: ContactType) => {
          if (el.id === id) {
            return contact;
          }
          return el;
            ...
            >
              <div className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-center align-middle transition-all transform bg-white shadow-xl rounded-2xl">
                  ...
                  <form onSubmit={handleSubmit}>
                    ...
                    <div className="mt-4 mb-2">
                      ...
                      <input
                        ...
                        name="status"
                        value="Active"
                        checked={filteredData[0]?.status === "Active"}
                        onChange={handleChange}
                      />
                      Active
                      <input
                        ...
                        type="radio"
                        name="status"
                        checked={filteredData[0]?.status === "Inactive"}
                        value="Inactive"
                        onChange={handleChange}
                      />
                      Inactive
                ...
                <div className="mt-4">
                  <button
                    type="button"
                    className="inline-flex justify-center px-4 py-2 text-sm font-mediumtext-grey-600 border bg-slate-600 border border-transparent rounded-md hover:bg-blue-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-blue-500"
                    onClick={closeModal}
                  >
                    Close
                  </button>
